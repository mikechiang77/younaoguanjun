...
Page({
  ...
  async onLoad() {                           //注意这里async是变异步为同步
    ...
    var user = wx.getStorageSync('user'); //取出全局变量user当前用户名 缓存？
    this.setData({ user: user });        //赋值给user
    var avatar = wx.getStorageSync('avatar'); //取出全局变量user当前用户头像 缓存？
...
this.setData({ beat_percent: parseInt(beat_percent) });    //计算领先百分比
   ...
   //如果数据库没有按题号和昵称的检索结果，就把当前用户的题号、用户名、答题用时添加到表score1的一条记录中去
   /*
   if (len === 0){                  //这个条件式列的对不对？
     const db=wx.cloud.database()        
     const table1=db.collection("score1")
  ...
  gotonext(){
    ...
    //取出全局变量记录数
    var sizenum = wx.getStorageSync('sizenum');
  ...
  onShareAppMessage: function () {
    return{
    title: this.data.user+'仅用时'+this.data.useminute+'分'+this.data.usesecond+'秒，答出第'+this.data.show_qnumber+'题, 来试试?', //qnumber显示undefined?
